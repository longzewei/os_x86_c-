...
interrupt_handler_%1:
    ; xchg bx, bx
%ifn %2
    push 0x20222202 ;随便压一个数占位,有些中断不压入error_code,有些压,所以平衡一下
%endif
    push %1; 压入中断向量(编号)，跳转到中断入口
    jmp interrupt_entry
%endmacro
...
interrupt_exit:
    ...
    ;弹出 error_code 和 中断编号(push 默认压入32位吗)
    add esp, 8
    ; 根据eflags确认的特权级,iret的表现不同,
    ; 但中断隐指令和iret之间的内容,是一致的
    iret
